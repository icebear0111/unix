...
void do_writer(int id, struct databuf2 *buf, int semid){
		...
		sem_wait(semid, 0); //semaphore 작업

		flag = buf->index;
                strcpy(buf->buf[flag].msg, temp); // msg write
                buf->buf[flag].s_id = id; // msg의 sender 설정
                buf->index = (buf->index + 1) % 10; // index update
		if (cnum==1)
			printf("id=%d, talkers=%d, msg#=%d...\n", id, cnum, buf->index);
                
		sem_signal(semid, 0); //semaphore 작업
		// semaphore 작업
                for (i = 0; i < 4; i++) {
                        if (buf->members[i] == 1)
...
int main(int argc, char** argv){
        ...
        ushort init[9] =  {1, 0, 0, 0, 0, 10, 10, 10, 10}; //semaphore 초기값
	key_t key1, key2;
	pid_t pid[2];
        union semun arg;
        struct databuf2 *buf;
